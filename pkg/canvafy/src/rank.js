...
module.exports = class Rank {
  ...
  */
  ...
  setCurrentXp(xp, color = "#000") {
    // Validate the xp value
    if (typeof xp !== "number" || xp < 0) {
      throw new Error("The first argument of setCurrentXp method must be a number greater than or equal to 0.");
    }

    // Validate the color value
    if (typeof color !== "string") {
      throw new Error("The second argument of setCurrentXp method must be a string.");
    }
    if (!/^#([a-fA-F0-9]{6}|[a-fA-F0-9]{3})$/.test(color)) {
      throw new Error("The second argument of setCurrentXp method must be a valid hexadecimal color.");
    }

    // Set the xp and color values
  ...
  async build() {
    ...
    ctx.textAlign = "start";
    ctx.font = `25px ${this.font.name} Bold`;
    ctx.fillStyle = this.current_xp.color;

    const currentXPText = this.current_xp.data === this.required_xp.data ? "MAX" : `${this.current_xp.data}`;
    ctx.fillText(currentXPText, 590, 161)

    ctx.fillStyle = this.required_xp.color;
    const requiredXPText = this.current_xp.data === this.required_xp.data ? "" : ` / ${this.required_xp.data}`;
    ctx.fillText(requiredXPText, 590 + ctx.measureText(currentXPText.toString()).width, 161)
    ctx.closePath();
